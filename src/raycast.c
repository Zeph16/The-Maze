...
float castRay(Vector rd)
{
    ...
    if (rd.x < 0) { dir.x = -1; raylength.x = (pPos.x - lasttile.x) * unitlength.x; }
    else          { dir.x =  1; raylength.x = (lasttile.x+boxSize - pPos.x) * unitlength.x; }
    if (rd.y < 0) { dir.y = -1; raylength.y = (pPos.y - lasttile.y) * unitlength.y; }
    else          { dir.y =  1; raylength.y = (lasttile.y+boxSize - pPos.y) * unitlength.y; }
